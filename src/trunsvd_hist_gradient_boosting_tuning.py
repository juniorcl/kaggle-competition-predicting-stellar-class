...
def tune_trunsvd_hist_gradient_boosting(X_train: pd.DataFrame, y_train: pd.DataFrame | pd.Series, target: str, model_path: str) -> tuple:

    logger = setup_model_logger('trunsvd_hist_gradient_boosting')
    setup_optuna_logger(logger)

    logger.info("----- Tuning TruncatedSVD + HistGradientBoosting -----")
