...
def tune_trunsvd_sgdclassifier(X_train: pd.DataFrame, y_train: pd.DataFrame | pd.Series, target: str, model_path: str) -> tuple:

    logger = setup_model_logger('trunsvd_sgdclassifier')
    setup_optuna_logger(logger)

    logger.info("----- Tuning TruncatedSVD + SGDClassifier -----")
