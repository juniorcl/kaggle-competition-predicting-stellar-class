...
def tune_trunsvd_rbfsampler_knn(X_train: pd.DataFrame, y_train: pd.DataFrame | pd.Series, target: str, model_path: str) -> tuple:

    logger = setup_model_logger('trunsvd_rbfsampler_knn')
    setup_optuna_logger(logger)

    logger.info("----- Tuning TruncatedSVD + RBFSampler + KNN -----")
