...
MODEL_PARAMS = ["alpha", "solver", "fit_intercept", "class_weight", "max_iter"]


def tune_trunsvd_rbfsampler_ridge(X_train: pd.DataFrame, y_train: pd.DataFrame | pd.Series, target: str, model_path: str) -> tuple:

    logger = setup_model_logger('trunsvd_rbfsampler_ridge')
    setup_optuna_logger(logger)

    logger.info("----- Tuning TruncatedSVD + RBFSampler + Ridge -----")
    ...
    def objective(trial, X, y):
        ...
        if solver in ["lsqr", "sag", "saga"]:
            model_params["max_iter"] = trial.suggest_int("max_iter", 500, 5000, step=500)
    ...
    best_rbf = {k: v for k, v in study.best_params.items() if k in RBFSAMPLER_PARAMS}
    best_model = {k: v for k, v in study.best_params.items() if k in MODEL_PARAMS}
    best_model.pop("max_iter", None)
    ...
    tuned_model = CalibratedClassifierCV(
        estimator=make_pipeline(
            ...
            StandardScaler(),
            RBFSampler(**best_rbf, random_state=42),
            StandardScaler(),
