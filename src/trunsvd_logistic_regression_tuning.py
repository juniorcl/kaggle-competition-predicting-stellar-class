...
def tune_trunsvd_logistic_regression(X_train: pd.DataFrame, y_train: pd.DataFrame | pd.Series, target: str, model_path: str) -> tuple:

    logger = setup_model_logger('trunsvd_logistic_regression')
    setup_optuna_logger(logger)


    logger.info("----- Tuning TruncatedSVD + Logistic Regression -----")
